...
- (BOOL)open:(NSString *)filename {
	...
	NSString *path = [[paths objectAtIndex:0] stringByAppendingFormat:@"/%@", filename];
	
	//NSLog(@"%@",path);
	if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
		//Copy from resource
		NSString *resourcedbpath = [[NSBundle mainBundle] pathForResource:filename ofType:nil];
		if ([[NSFileManager defaultManager] fileExistsAtPath:resourcedbpath]) {
			[[NSFileManager defaultManager] copyItemAtPath:resourcedbpath toPath:path error:nil];
		}
	}
...
- (id)columnData:(sqlite3_stmt *)stmt columnIndex:(NSInteger)index {
	...
	if (columnType == SQLITE_TEXT) {
		...
		NSString *cntext = [NSString stringWithUTF8String:(char *)sqlite3_column_text(stmt, index)];
		return cntext;
	}
