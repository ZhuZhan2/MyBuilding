...
-(void)myCompanyAction{
    if([[LoginSqlite getdata:@"userId"] isEqualToString:@""]){
        ...
        if(self.isHasCompany){
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:@"你还未申请加入企业或申请还没被批准，赶快搜索企业进行申请吧！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
        }
    }
}
